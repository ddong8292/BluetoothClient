...
public class MainActivity extends AppCompatActivity {
    ...
    BluetoothAdapter bluetoothAdapter;//장치제어
   ...
   ------------------------------------------------------------- */
        ...
        switch (requestCode){
            case REQUEST_ACCESS_PERMISSION:
                if(permissions.length>0&&grantResults[0]==PackageManager.PERMISSION_DENIED){
                    showMsg("안내","권한을 부여하지 않으면 앱 사용이 불가합니다.");
                }
        }
    }
    public void checkAccessPermission(){
        //로케이션 권한체크! 새로운핸드폰을 위한 처리(4.xx 초과버전에서 이 권한이 필요)!!
    ...
    public void scanDevice() {
        //시스템의 브로드 캐스팅을 낚아채서 알맞는 처리
        BroadcastReceiver receiver = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                //수많은 방송내용중 우리가 등록한 Action_Found에 대해서만 반응을 보내자!
